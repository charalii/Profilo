...
class EUISScraper(BaseScraper):
    """Scraper for EU Institute for Security Studies opportunities."""
            ...
            for selector in selectors:
                for link in soup.select(selector):
                    ...
                    if not href.startswith("http"):
                        href = f"https://www.iss.europa.eu{href}"
                    ...
                    try:
                        ...
                        body = detail.select_one("main, .content, article, .body")
                        desc = (
                    ...
                    listings.append(
                        {
                            "title": title,
                            "organization": "EU Institute for Security Studies",
                            "url": href,
                            "description": desc,
                            "contract_type": self._detect_contract_type(desc),
                            "location": self._find_location(desc) or "Paris",
                        }
                    )
